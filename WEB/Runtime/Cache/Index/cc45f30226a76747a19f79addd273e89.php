    ...
    <section class="service-section normal-layer no-padding-layer i-border-image">
        <ul class="feature-list interaction-list">
            <li class="feature-item">                
                <p class="inner-bar"><span class="order-text">我的评论<em id="orderInfo"></em></span></p>
            </li>
			<?php if(is_array($ping)): foreach($ping as $key=>$v): ?><li>
				<div class="padding5">
				<p <?php if($v[dis]): ?>class="green"<?php else: ?>class="gray"<?php endif; ?>><?php echo ($v["neir"]); ?></p>
				<p><a href="<?php echo U(MODULE_NAME.'/'.$v['fz']);?>">查看评论文章</a>&nbsp;&nbsp;&nbsp;&nbsp;评论时间：<span style="font-size:12px;color:gray;"><?php echo (date('Y-m-d H:i:s',$v["time"])); ?></span></p>
				<?php if(is_array($v["hf"])): foreach($v["hf"] as $key=>$h): ?><p class="red"><b>回复：</b><?php echo ($h["neir"]); ?></p><?php endforeach; endif; ?>
				</div>
