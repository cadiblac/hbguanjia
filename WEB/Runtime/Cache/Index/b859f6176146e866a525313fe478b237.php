    ...
    <section class="masthead-section no-padding-layer no-border-layer">
        ...
        <div class="masthead-login">
            ...
            <div class="masthead-caption">
                ...
                <p class="masthead-setting"><?php if(!$member['state']): ?><span style="color:red;">认证送积分</span>&nbsp;-->&nbsp;<?php endif; ?><a href="/index.php/Index/member/person/">完善资料<i class="icon icon-right"></i></a></p>
            </div>
        </div>
    </section>
    ...
    <section class="service-section normal-layer no-padding-layer i-border-image">
        <ul class="feature-list interaction-list">
			...
			<?php if(is_array($orders)): foreach($orders as $key=>$v): ?><li>
				...
				<?php default: ?>处理中<?php endswitch;?></span></p></div>
				...
				<p><?php if($v['status'] == 1): ?><a href="<?php echo U('/Index/orders/del',array('id'=>$v['id']));?>">删除</a><a class="zd" href="<?php echo U('/Index/orders/pay',array('id'=>$v['id']));?>">去支付</a>
				<?php else: ?>
				<a class="zd" href="<?php echo U('/Index/orders/updata',array('id'=>$v['id']));?>">完成订单</a><?php endif; ?></p>
				</div>
