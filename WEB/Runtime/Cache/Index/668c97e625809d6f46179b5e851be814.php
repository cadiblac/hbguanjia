    ...
    <section class="service-section normal-layer no-padding-layer i-border-image">
        <ul class="feature-list interaction-list">
            <li class="feature-item">                
                <p class="inner-bar"><span class="order-text">我的订单<em id="orderInfo"></em></span></p>
            </li>
			<?php if(is_array($orders)): foreach($orders as $key=>$v): ?><li>
				...
				<?php if($v['status'] == 2 or $v['status'] == 3): ?><p style="text-align:right;">				
				<a class="zd" href="<?php echo U('/Index/orders/updata',array('id'=>$v['id']));?>">完成订单</a>
				</p><?php endif; ?>
				</div>
