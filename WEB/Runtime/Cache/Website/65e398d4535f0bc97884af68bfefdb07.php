							...
							<form class="cmxform form-horizontal adminex-form" method="post" action="<?php echo U(MODULE_NAME.'/Article/runAdd');?>">
								...
								<div class="form-group ">
									...
									<?php if($cid == 1): ?><select class=" form-control" name="author">
										...
										<?php if(is_array($staff)): foreach($staff as $key=>$s): ?><option value="<?php echo ($s["id"]); ?>"><?php echo ($s["name"]); ?></option><?php endforeach; endif; ?>
										</select>
									<?php else: ?>
										<input class="form-control " type="text" name="author" value="<?php echo session('name');?>" /><?php endif; ?>
									</div>
								</div>
								<div class="form-group ">
