                ...
                <div class="col-md-12">
                    <section class="panel">
                        ...
                        <div class="panel-body">
							...
							<form action="<?php echo U(MODULE_NAME.'/Cate/sortCate');?>" method="post">
								<div class="clearfix">
								<?php if($_SESSION['username'] == C('RBAC_SUPERADMIN')): ?><div class="form-group">
										<a href="<?php echo U(MODULE_NAME.'/Cate/addCate');?>" class="btn btn-primary">
											添加一级栏目 <i class="fa fa-plus"></i>
										</a>
									</div><?php endif; ?>
								</div>
								...
								<tbody>
								<?php if(is_array($cate)): foreach($cate as $key=>$v): ?><tr class="">
									<td><?php echo ($v["id"]); ?></td>
									...
									<td>
										<?php switch($v["model"]): case "Article": ?>文章模型<?php break;?>
											...
											<?php default: ?>未选择模型<?php endswitch;?>
									</td>
									<td><?php echo ($v["level"]); ?></td>
									<td><input type="text"  name="<?php echo ($v["id"]); ?>" value="<?php echo ($v["sort"]); ?>" /></td>
									<td>
										<a href="<?php echo U(MODULE_NAME.'/Cate/updataCate',array('id'=>$v['id']));?>"><span class="btn btn-success btn-xs">修改</span></a>
									<?php if($_SESSION['username'] == C('RBAC_SUPERADMIN')): ?><a href="<?php echo U(MODULE_NAME.'/Cate/addCate',array('pid'=>$v['id']));?>"><span class="btn btn-info btn-xs">添加子项</span></a>
										<a onclick="return confirm('确实要删除吗？')" href="<?php echo U(MODULE_NAME.'/Cate/delCate',array('id'=>$v['id']));?>"><span class="btn btn-warning btn-xs">删除</span></a><?php endif; ?>
