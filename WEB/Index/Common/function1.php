...
//检测用户信息
	function Check(){
		...
		$model=M('member');
		if($rst=$model->where(array('openid'=>$data))->select()){
			//设置SESSION
			setSession($rst[0]);
			return true;
		}else{
	...
	function getUserInfo(){
		if(!$_GET['code']){
			...
			if($mid=$mode->where(array('openid'=>$userInfo['openid']))->getField('id')){
				$sj=array('openid'=>$userInfo['openid'],'username'=>$userInfo['nickname'],'sex'=>$userInfo['sex'],'photo'=>$userInfo['headimgurl'],'yqrid'=>$spread,'logintime'=>time());
				$mode->save($sj);
			}else{
